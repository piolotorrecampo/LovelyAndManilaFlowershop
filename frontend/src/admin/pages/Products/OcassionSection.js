...
import { GetOcassions, DeleteOcassion, AddOccassion } from '../../../FetchData'
...
export default function OcassionSection(props) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await AddOccassion(items)
      console.log(res);
      alert('added');
      setPhoto(null)
      setTitle('')
          ...
          {list &&
            list.map((item) => (
              <div className="justify-between p-3 items-center flex border flex-row gap-10">
                ...
                <button className='px-5' onClick={() => DeleteOcassion(item._id)}><DeleteIcon/></button>  
              </div>
            ))}
        </div>
        <div className='w-1/2 md:w-full'>
          <h2 className='text-2xl pb-5'>Add Ocassion</h2>
